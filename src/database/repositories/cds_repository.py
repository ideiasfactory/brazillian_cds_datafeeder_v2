...
class CDSRepository:
    ...
    async def bulk_insert(
        ...
        records_with_source = [
            {**rec, "source": source} for rec in records
        ]
    ...
    async def get_recent_logs(self, limit: int = 10) -> List[DataUpdateLog]:
        ...
        """
        stmt = select(DataUpdateLog).order_by(desc(DataUpdateLog.timestamp)).limit(limit)
        result = await self.session.execute(stmt)
        return list(result.scalars().all())
    ...
    async def delete_by_date_range(
        ...
        stmt = delete(CDSRecord).where(
            and_(
                CDSRecord.date >= start_date,
                CDSRecord.date <= end_date
            )
        )
