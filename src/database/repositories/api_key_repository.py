...
class APIKeyRepository:
    ...
    async def validate_key(self, key: str) -> Optional[APIKey]:
        ...
        # Check expiration
        if api_key_record.expires_at is not None and datetime.utcnow() > api_key_record.expires_at.replace(tzinfo=None):
        ...
        logger.info(f"Valid API key used: {api_key_record.name} (requests: {api_key_record.request_count})")
