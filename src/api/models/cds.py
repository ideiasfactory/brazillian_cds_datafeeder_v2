...
class StandardResponse(BaseModel):
    ...
    error_message: Optional[str] = Field(
        None, description="Error message (only present on errors)"
    )
...
class CDSSchemaInfo(BaseModel):
    ...
    update_frequency: str = Field(
        "Daily at 06:00 UTC (03:00 BRT)", description="Data update schedule"
    )
    currency: str = Field("USD", description="Currency for CDS spreads")
    unit: str = Field(
        "Basis points (1 bp = 0.01%)", description="Unit of measurement for prices"
    )
...
class CDSInfoData(BaseModel):
    ...
    schema_info: CDSSchemaInfo = Field(
        ..., description="Schema documentation", alias="schema"
    )
    statistics: Optional[CDSStatisticsInfo] = Field(
        None, description="Database statistics (only when include_stats=true)"
    )
